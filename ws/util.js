...
function show_results_box_title(timestamp, stellarbeat_timestamp) {
	...
	create_title_in_div("Results for " + datestring + " (crawl data has timestamp " + stellarbeat_timestamp + ")");
}

function show_mqs_results(mqs, mqs_len, has_quorum_intersection) {
	var mqs_output = JSON.stringify(mqs, null, 4);
        for (let l = 0; l < window.nodes.length; l++){mqs_output = mqs_output.replaceAll(window.nodes[l].publicKey, window.nodes[l].hostname);}
...
function show_mbs_results(mbs, mbs_len, mbs_min) {
	...
	var mbs_tooltip = "Control over any of these sets is sufficient to compromise the liveness of all nodes and to censor future transactions.";        
        for (let l = 0; l < window.nodes.length; l++){mbs_output = mbs_output.replaceAll(window.nodes[l].publicKey, window.nodes[l].hostname);}
...
function show_mss_results(mss, mss_len, mss_min) {
	var mss_output = JSON.stringify(mss, null, 4);
        for (let l = 0; l < window.nodes.length; l++){mss_output = mss_output.replaceAll(window.nodes[l].publicKey, window.nodes[l].hostname);}
